...
class LoReGro:
    def __init__(self, im, seeds, im_path=None, energy=None, ball_r_out=1, ball_r=3, min_diff=0.1, alpha=1,
                 scale=0.5, max_iters=1000, smoothing=True):
...
if __name__ == '__main__':
    ...
    _, seeds = tools.initialize_graycom(data)
    seeds = skimor.binary_erosion(seeds, np.ones((3, 3))).astype(np.uint8)

    # plt.figure()
    # plt.subplot(121), plt.imshow(data, 'gray')
    # plt.subplot(122), plt.imshow(seeds, 'jet')
    # plt.show()

    lrg = LoReGro(data, seeds, ball_r_out=ball_r_out, ball_r=ball_r, min_diff=min_diff, alpha=alpha,
                  scale=scale, max_iters=max_iters, smoothing=smoothing)
